...
%phi_rot: rotation angle of ellipsoid along x,y
%ni,ns: refractive index of medium and particle respectively
%lambda: wavelength of light
%nu: internal (1) or external (3)
%

function [J11,J12,J21,J22,dJ11,dJ12,dJ21,dJ22] = compute_J_ellip_celes_posm_angle(lmax,Ntheta,Nphi,a,b,c,phi_rot,ni,ns,lambda,nu)
tic
...
J21 = zeros(nmax/2);
J22 = zeros(nmax/2);
%preallocate for dJ (nmax/2 x nmax/2 x 3) 1:a, 2:b, 3:c, 4:phi
dJ11 = zeros(nmax/2,nmax/2,4);
dJ12 = zeros(nmax/2,nmax/2,4);
dJ21 = zeros(nmax/2,nmax/2,4);
dJ22 = zeros(nmax/2,nmax/2,4);
%setup for gaussian quadrature in two dimensions
[theta_i,wtheta_i] = generate_gauss_weights_abscissae(Ntheta,0,pi/2);
[phi_i,wphi_i] = generate_gauss_weights_abscissae(Nphi,phi_rot,pi+phi_rot);
        ...
        for lp = 1:lmax
            ...
            for mp = -lp:lp
                ...
                if selection_rules_1122 ~= 0
                    ...
                    dJ11(ni,np,4) = sum(sum(dJ11_r.*radius_grad(:,:,4)+1i*(-mp+mi)*weight_map.*prefactor.*J11_r));
                    ...
                    dJ22(ni,np,4) = sum(sum(1i*(-mp+mi)*weight_map.*prefactor.*(J22_r+J22_t.*ellip1+J22_p.*ellip2)+weight_map.*prefactor.*((dJ22_r+dJ22_t+dJ22_p).*radius_grad(:,:,4)+J22_t.*dellip1(:,:,4)+J22_p.*dellip2(:,:,4))));
                end
                %compute J12,J21
                if selection_rules_1221 ~= 0
                    ...
                    dJ12(ni,np,4) = sum(sum(1i*(-mp+mi)*weight_map.*prefactor.*(term1+term2)+weight_map.*prefactor.*((dJ12_r+(ang_t.*ellip1+ang_p.*ellip2).*d_r3jb).*radius_grad(:,:,4)+r3jb.*(ang_t.*dellip1(:,:,4)+ang_p.*dellip2(:,:,4)))));
                    ...
                    dJ21(ni,np,4) = sum(sum(1i*(-mp+mi)*weight_map.*prefactor.*(term1+term2)+weight_map.*prefactor.*((dJ21_r+(ang_t.*ellip1+ang_p.*ellip2).*d_r3jb).*radius_grad(:,:,4)+r3jb.*(ang_t.*dellip1(:,:,4)+ang_p.*dellip2(:,:,4)))));
                end
            end
        end
    end
